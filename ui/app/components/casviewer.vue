    ...
    TabPanels
      TabPanel(value="detailed")
        ...
        Panel.pb-4(v-for="folio in folios" :key="folio.folio" :toggleable="true")
          ...
          DataTable(
v-for="scheme in getSchemes(folio)" :key="scheme.scheme" size="small"
      ...
      TabPanel(value="gains")
        ...
        .pb-4(v-else)
          Panel(v-for="(fy, index) in fys" :key="fy" toggleable :collapsed="index > 0")
            ...
            DataTable.my-4(
v-for="fund in gains[fy].funds" :key="fund.fund.isin" size="small"
...
watch(cas, (value: CASParserData | null) => {
  ...
  value.folios.forEach((folio) => {
    folio.schemes.forEach((scheme) => {
      // API serializes Decimal values as strings
      valuation.value += Number(scheme.valuation.value)
    })
  })
})
...
.p-datatable th[class*="text-"] .p-datatable-column-header-content {
  display: inline-flex;
}
</style>
