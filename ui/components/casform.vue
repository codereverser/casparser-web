        ...
        Card
            ...
            template(#content)
                .flex.flex-row.justify-content-end.file-select
                    InputText.mr-4.col-8(v-model="filename" placeholder="Select a file" :disabled="true")
                    Button.mr-4.file-select(label="Select" @click="selectFile")
                    input(ref="file" type="file" :disabled="false" accept="application/pdf" @change="fileSelected" )
            template(#footer)
